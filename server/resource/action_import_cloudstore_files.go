...
func (d *importCloudStoreFilesPerformer) DoAction(request Outcome, inFields map[string]interface{}, transaction *sqlx.Tx) (api2go.Responder, []ActionResponse, []error) {
	...
	for colName, colFkdata := range cloudStores {
		...
		userId, err := d.cruds[USER_ACCOUNT_TABLE_NAME].GetReferenceIdToId(USER_ACCOUNT_TABLE_NAME, cacheFolder.CloudStore.UserId, transaction)
		...
		if cacheFolder.CloudStore.CredentialName != "" {
			...
			if cred.DataMap != nil {
				for key, val := range cred.DataMap {
					config.Data().SetValue(cacheFolder.CloudStore.CredentialName, key, fmt.Sprintf("%s", val))
				}
			}
		}
