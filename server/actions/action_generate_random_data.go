...
func (actionPerformer *randomDataGeneratePerformer) DoAction(request actionresponse.Outcome, inFields map[string]interface{}, transaction *sqlx.Tx) (api2go.Responder, []actionresponse.ActionResponse, []error) {
...
func GetFakeRow(columns []api2go.ColumnInfo) map[string]interface{} {
	...
	for _, col := range columns {
		...
		for _, c := range resource.StandardColumns {
		...
		fakeValue := resource.ColumnManager.GetFakeData(col.ColumnType)
...
func NewRandomDataGeneratePerformer(initConfig *resource.CmsConfig, cruds map[string]*resource.DbResource) (actionresponse.ActionPerformerInterface, error) {
