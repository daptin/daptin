...
| Deterministic verification | Passing: both repositories' full normal, race, and vet suites; module vulnerability and dependency-boundary checks; protocol/adapter fixtures; bounded JSON/SSE/provider-response/tool-argument fuzzing; routing/fallback; cache isolation/coalescing; admitted-request cancellation; lifecycle; architecture scans; two-host catalog fan-out; registered action E2E; and the Daptin database matrix. The Daptin root harness now keeps the Olric start error goroutine-local and reports FTP failures directly instead of terminating before its diagnostic; clean full runs pass in 78.9 seconds normal and 165.5 seconds under `-race` with `GOWORK=off`. |
| Live certification | Partially complete; the manifest remains `kind: target`. The real-provider matrix passed all 8 Google cells, all 10 Lilac cells, and 11 of 12 OpenRouter cells across chat, streaming, tools, structured output, vision, embeddings, Responses, and image generation. OpenRouter image generation remains an explicit non-pass because the supplied account returned HTTP 402 `insufficient_quota`; a skip or account failure is not certification. A transient Google embedding 429 was classified as `rate_limit_error` and passed on targeted retry after its quota window. No credential or provider payload is stored in either repository. |
...
| Release reproducibility | Complete. Module commit `db40b5d` is published as `v0.1.0-dev.30`, and its exact tagged SHA passes the hosted [module CI run](https://github.com/daptin/llmgateway/actions/runs/33478482489), including normal, race, vet, live-tag compilation, vulnerability, single-module, and dependency-boundary checks. Daptin resolves the published version without a `replace`; `GOWORK=off go mod tidy`, the full normal and race suites, and repository-wide vet pass with the published checksum. |
