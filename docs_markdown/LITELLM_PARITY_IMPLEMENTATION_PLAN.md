...
| Release reproducibility | The last published baseline is module commit `db40b5d` at `v0.1.0-dev.30`, whose exact tagged SHA passes the hosted [module CI run](https://github.com/daptin/llmgateway/actions/runs/33478482489). Production hardening commit `a722bc1` passes local normal, race, vet, fuzz-smoke, vulnerability, single-module, dependency-boundary, and Daptin integration gates; `ee41f2e` adds the compiled live credential-rejection cells. The current module head is intentionally not yet tagged or pinned. A fresh live run, publishing, and `GOWORK=off` verification of the new tag remain release gates. |
