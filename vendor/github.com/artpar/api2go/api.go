...
func (p paginationQueryParams) getLinks(r *http.Request, count uint, info information) (result jsonapi.Links, err error) {
  ...
  if p.number != "" {
    ...
    result["current_page"] = number
    result["last_page"] = totalPages
    result["from"] = (number - 1) * size
...
func (api *API) addResource(prototype jsonapi.MarshalIdentifier, source CRUD) *resource {
  ...
  } else if resourceType.Kind() == reflect.Map {
    ptrPrototype = reflect.MakeMap(resourceType).Interface()
  } else {
...
func (res *resource) handleIndex(c APIContexter, w http.ResponseWriter, r *http.Request, info information) error {
  //res.source.(PaginatedFindAll).PaginatedFindAll(buildRequest(c, r))
  if source, ok := res.source.(PaginatedFindAll); ok {
    //fmt.Printf("handle index: %v\n : %v\n", reflect.TypeOf(res.source))
    ...
    //fmt.Printf("pagination links: %v", paginationLinks)
